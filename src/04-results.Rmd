...
ivs_multi_f1 <- ivst %>%
  ...
  pivot_wider(names_from = "model", values_from = "median_f1")
...
# F1-score summary
ivs_f1_summary <- list(ivs_multi_f1,
                       ivs_2S_f1,
                       ivs_seq_f1) %>%
...
```{r f1-ranks}
f1_ranks <- ivs_f1_summary %>% 
  pivot_longer(cols = -class, names_to = "model", values_to = "f1") %>% 
  group_by(class) %>% 
  mutate(rank = factor(row_number(-f1))) %>% 
  ungroup() %>% 
  select(-f1) %>% 
  arrange(rank) %>% 
...
Out of the `r ncol(f1_ranks)` methods (algorithm-sampling combinations), the top 10 aggregated list comprise of: `r paste(head(f1_top[["top.list"]], 10), collapse = ", ")`.
