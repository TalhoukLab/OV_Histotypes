...
```{r gene_2s, fig.cap='Gene Optimzation for Two-Step Classifier'}
dat_gene2s <- gene2s %>% 
  filter(
...
p_gene_2s <- ggplot(dat_gene2s, aes(x = Genes, y = `50%`)) +
  ...
  labs(x = "Genes Added",
       y = "Median",
       title = "Gene Optimzation for Two-Step Classifier")
print(p_gene_2s)
```
...
- **adaboost-up**: adaboost algorithm with upsampling. The sequence of models and algorithms used are: 
...
conf_step1_pred <-
  prediction(mod_two_step[[1]], conf_step1_data, conf_step1_class)
conf_step1_eval <-
  evaluation(conf_step1_class, conf_step1_pred)

conf_step2_pred <-
  prediction(mod_two_step[[2]], conf_step2_data, conf_step2_class)
conf_step2_eval <-
  evaluation(conf_step2_class, conf_step2_pred)
...
conf_summary <- bind_rows(conf_2S_eval,
                          conf_seq_eval,
                          conf_full_eval)
kable(conf_summary,
...
In the confirmation set, **sequential** improves drastically in LGSC classification compared to **adaboost-up**, with moderate improvement in ENOC, and minor improvement in CCOC. **sequential** improves on **two_step** in all classes.
