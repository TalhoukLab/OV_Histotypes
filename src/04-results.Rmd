...
# Top method: 2-Step classifier using random forest algorithm and no subsampling
mod_step1 <- classification(
...
# Train using top sequential method ---------------------------------------

# Sequential data
samp <- "none"
seq_data <- readRDS(here("data", "seq_data.rds"))[[samp]]
seq_class <- readRDS(here("data", "seq_class.rds"))[[samp]]
seq_algs <- readRDS(here("data", "seq_algs.rds"))[[samp]]

mod_seq <- pmap(list(seq_data, seq_class, seq_algs), ~ {
  classification(
    data = ..1,
    class = ..2,
    algorithm = ..3,
...
# Train using multinomial equivalent of top method ------------------------
...
conf_step1_pred <- prediction(mod_step1, conf_step1_data, conf_step1_class)
conf_step1_eval <- evaluation(conf_step1_class, conf_step1_pred)
conf_step2_pred <- prediction(mod_step2, conf_step2_data, conf_step2_class)
conf_step2_eval <- evaluation(conf_step2_class, conf_step2_pred)

# Combine evaluations from both steps
conf_2S_eval <- list(conf_step1_eval, conf_step2_eval) %>%
  map("cs") %>%
  unlist() %>%
...
seq_conf_data <- map(seq_conf_ranks, ~ {
  conf_df %>%
...
# Predict and evaluate
conf_seq_eval <-
  pmap(list(mod_seq, seq_conf_data, seq_conf_class), ~ {
...
conf_summary <- bind_rows(conf_2S_eval, conf_seq_eval, conf_full_eval)
kable(conf_summary,
...
# Predict and evaluate
val_step1_pred <- prediction(mod_step1, val_step1_data, val_step1_class)
val_step1_eval <- evaluation(val_step1_class, val_step1_pred)
val_step2_pred <- prediction(mod_step2, val_step2_data, val_step2_class)
val_step2_eval <- evaluation(val_step2_class, val_step2_pred)
...
val_summary <- bind_rows(val_2S_eval, val_seq_eval, val_full_eval)
kable(val_summary,
