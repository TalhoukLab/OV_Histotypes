...
geneseq_names <-
  enframe(ranked_vi_seq, name = "Genes", value = "Name")

dat_geneseq <- geneseq %>% 
  filter(
    (measure == "f1.HGSC" & Sequence == 1) | 
      (measure == "f1.CCOC" & Sequence == 2) |
      (measure == "f1.ENOC" & Sequence == 3) | 
      (measure %in% c("f1.MUC", "f1.LGSC") & Sequence == 4)
  ) %>% 
  mutate(
  ...
  inner_join(geneseq_names, by = "Genes")

p_gene_seq <-
  ggplot(dat_geneseq, aes(x = Genes, y = `50%`, text = Name)) +
...
n_genes_seq <- 34
genes_add_seq <- ranked_vi_seq[seq_len(n_genes_seq)]
...
gene2s_names <-
  enframe(ranked_vi_2s, name = "Genes", value = "Name")
...
dat_gene2s <- gene2s %>% 
  ...
  inner_join(gene2s_names, by = "Genes")

p_gene_2s <-
  ggplot(dat_gene2s, aes(x = Genes, y = `50%`, text = Name)) +
  facet_wrap(~ label, ncol = 1, scales = "free_y") +
  geom_point() +
  theme_bw() +
  labs(x = "Genes Added",
...
n_genes_2s <- 24
genes_add_2s <- ranked_vi_2s[seq_len(n_genes_2s)]
...
```{r conf-eval-overall}
conf_2S_eval_overall <- list(
  two_step_full_1 = conf_step1_eval_full,
  two_step_full_2 = conf_step2_eval_full,
  two_step_optimal_1 = conf_step1_eval_opt,
  two_step_optimal_2 = conf_step2_eval_opt
) %>%
  map(`[`, c("auc", "accuracy", "macro_f1", "kappa", "gmean")) %>%
  bind_rows(.id = "Step") %>%
  separate_wider_regex(Step, c(method = ".*", "_", step = ".$"))

conf_seq_eval_overall <- map(list(
  sequential_full = list(mod_seq_full, seq_conf_data_full, seq_conf_class),
  sequential_optimal = list(mod_seq_opt, seq_conf_data_opt, seq_conf_class)
), ~ {
  pmap(.x, ~ {
    pred <- prediction(..1, ..2, ..3)
    eval <- evaluation(..3, pred)
...
conf_eval_overall <-
  bind_rows(conf_2S_eval_overall, conf_seq_eval_overall)
