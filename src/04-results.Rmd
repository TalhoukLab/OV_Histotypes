...
p <- ggplot(
  ...
  aes(x = fct_reorder(wflow, mean_estimate, .desc = TRUE), y = mean_estimate)
) +
  geom_point() +
  theme_bw() +
...
overall_accuracy_train <-
  ...
  mutate(
    ...
    mean_estimate = ifelse(
      ...
      cell_spec(mean_estimate, bold = TRUE),
      mean_estimate
    )
  ) %>% 
  arrange(samp) %>% 
...
```{r train-accuracy-class, fig.cap='Training Set Class-Specific Mean Accuracy', fig.width=8, fig.height=8}
p <- ggplot(
  all_metrics_train %>%
    filter(class_group != "Overall", .metric == "accuracy"),
  aes(x = fct_reorder(wflow, mean_estimate, .desc = TRUE), y = mean_estimate)
...
```{r train-f1-class, fig.cap='Training Set Class-Specific Mean F1-Score', fig.width=8}
p <- ggplot(
  all_metrics_train %>%
    filter(class_group != "Overall", .metric == "f_meas"),
  aes(x = fct_reorder(wflow, mean_estimate, .desc = TRUE, .na_rm = FALSE), y = mean_estimate)
) +
  geom_point() +
  facet_wrap(~ class_group, ncol = 1) +
  theme_bw() +
...
```{r train-kappa-class, fig.cap='Training Set Class-Specific Mean Kappa', fig.width=8}
p <- ggplot(
  all_metrics_train %>%
    filter(class_group != "Overall", .metric == "kap"),
  aes(x = fct_reorder(wflow, mean_estimate, .desc = TRUE, .na_rm = FALSE), y = mean_estimate)
...
```{r train-gmean-class, fig.cap='Training Set Class-Specific Mean G-mean', fig.width=8}
p <- ggplot(
  all_metrics_train %>%
    filter(class_group != "Overall", .metric == "gmean"),
  aes(x = fct_reorder(wflow, mean_estimate, .desc = TRUE, .na_rm = FALSE), y = mean_estimate)
...
kable(class_gmean_train,
      caption = "raining Set Class-Specific Mean G-mean",
      escape = FALSE) %>%
  kable_styling()
```
