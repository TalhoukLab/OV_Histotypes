...
```{r codeset-counts}
n_coh <-
  lengths(list(cs1_samples_coh, cs2_samples_coh, cs3_samples_coh))
...
codeset_counts <- list(
  ...
  `Combine CS3 Training with AOC/USC sites` = n_train,
  `Major Histotypes` = n_train_hist,
  `Deduplicated` = n_train_uniq
) %>%
...
```{r train-norm-codeset}
train_norm_codeset <- hist %>%
  filter(FileName %in% gsub("^X", "", c(cs1_samples_X, cs2_samples_X, cs3_samples_X))) %>%
  mutate(revHist = revHist %>%
           factor(levels = c("HGSC", "CCOC", "ENOC", "MUC", "LGSC")) %>%
           fct_na_value_to_level(level = "Other")) %>%
  describeBy(
...
```{r train-cs3-processing-codeset}
train_cs3_processing_codeset <- 
...
hist_codeset <-
  bind_rows(
    ...
    train_norm_codeset,
    train_cs3_processing_codeset,
    train_all_hist_codeset,
    train_dedup_codeset
  )

hist_codeset %>%
  kbl(caption = "Histotype Distribution by CodeSet and Processing Stage") %>%
  kable_styling() %>%
  pack_rows(
    index = c(
      "Selected Cohorts" = 7,
      "QC" = 7,
      "Normalized to Reference" = 7,
      "Combine CS3 Training with AOC/USC sites" = 7,
      "Major Histotypes" = 6,
      "Deduplicated" = 6
    )
  ) %>%
