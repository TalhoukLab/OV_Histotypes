...
n_train_uniq <- train_ref_all %>% count(CodeSet) %>% deframe()
n_train_hist <- train_ref %>% count(CodeSet) %>% deframe()
...
cs3_train_samples_preqc <- cohorts %>%
  filter(
    file_source == "cs3",
...
preqc_df <-
  list(list(cs1, cs2, cs3),
       list(cs1_samples_coh, cs2_samples_coh, cs3_train_samples_preqc)) %>%
  pmap( ~ {
    ..1 %>%
