...
```{r classifier-hist}
source(here::here("validation/cs_classifier.R"))
```

```{r train-hist}
train_hist <- count(train_ref, revHist, sort = TRUE)

knitr::kable(train_hist, caption = "Training Set Histotype Distribution")
```

```{r train-hist-codeset}
train_hist_codeset <- train_ref %>%
    count(revHist, CodeSet, sort = TRUE) %>% 
    arrange(CodeSet) %>% 
    pivot_wider(names_from = "CodeSet", values_from = "n")

knitr::kable(train_hist_codeset, caption = "Training Set Histotype Distribution by CodeSet")
```
