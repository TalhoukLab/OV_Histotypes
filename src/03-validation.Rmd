...
```{r cs13-genes-topbot, fig.cap='Pairwise Top/Bottom 3 Genes between CS1 Data vs. CS3'}
...
cs13_genes_topbot <- counts1v3_df %>% 
  group_by(Gene) %>% 
  summarize(R2 = cor(`CS1-Non`, CS3) ^ 2) %>%
  filter(dense_rank(R2) <= 3 | dense_rank(desc(R2)) <= 3) %>% 
  mutate(R2 = paste0("R2 = ", round(R2, 3)))

counts1v3_df_topbot <- counts1v3_df %>% 
  inner_join(cs13_genes_topbot, by = "Gene") %>% 
...
p_cs13_genes_topbot <-
  ...
  geom_text(aes(label = R2), x = -20, y = 2, size = 3, 
            hjust = 0, check_overlap = TRUE) +
  facet_grid(rows = vars(Norm), cols = vars(Gene)) +
  labs(title = "Pairwise Top/Bottom 3 Genes between CS1 Data vs. CS3") +
  theme_bw() +
  theme(panel.grid.minor = element_blank())
