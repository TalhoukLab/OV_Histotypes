...
cs4_dat <- HKnorm(cs4$raw) %>% select(-any_of(cs4_qc_failed))
cs4_pools <- cs4_dat %>% 
...
# Common genes
cs345_common_samples <- hist_all %>%
  filter(ottaID %in% intersect(
    gsub("-N1|\\.(x|y)", "", names(cs4_dat)), names(cs5_dat)
  )) %>%
  pull(ottaID) %>%
  unique()
...
sub_pools5_BA <- counts5_BA %>%
  map(dim) %>%
  unique() %>%
  unlist() %>%
  paste0(c("Samples=", "Genes="), ., collapse = ", ")
...
cs3_overlap_cs45 <-
  inner_join(annot_all, cs4_overlap_cs5, by = "ottaID") %>%
  filter(CodeSet == "CS3",
         ottaID %in% intersect(
           gsub("-N1|\\.(x|y)", "", names(cs4_dat)), names(cs5_dat)
         )) %>%
  ...
  count(CodeSet, revHist) %>%
  pivot_wider(names_from = "CodeSet", values_from = "n") %>% 
  mutate(CS4 = CS3, CS5 = CS3)
knitr::kable(cs3_overlap_cs45, caption = "CS3/CS4/CS5 Common Samples Histotype Distribution")
```
