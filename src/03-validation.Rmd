...
```{r cc-rand1-cs23-meds, fig.cap='CS2 vs. CS3 Random1 Normalized 100 Runs of Median Concordance Measures'}
hist_common <- hist %>%
  filter(FileName %in% c(cs1_clean$FileName, cs2_clean$FileName, cs3_clean$FileName)) %>%
  distinct(ottaID, revHist)

hist_rand1_list <- map(seq_len(100), ~ {
  set.seed(.x)
  hist_common  %>%
    group_by(revHist) %>% 
    slice_sample(n = 1) %>%
...
metrics_runs_cs23 <- imap_dfr(hist_rand1_list, ~ {
  cs2_rand1 <- join_avg(cs2_clean, .x, "ottaID", "keep")
  cs3_rand1 <- join_avg(cs3_clean, .x, "ottaID", "keep")
  cs2_counts1 <- join_avg(cs2_clean, .x, "ottaID", "discard")
  cs3_counts1 <- join_avg(cs3_clean, .x, "ottaID", "discard")
  cs2_norm_rand1 <- as.data.frame(refMethod(cs2_counts1, cs3_rand1, cs2_rand1))
  
  pmap_dfr(list(cs2_norm_rand1, cs3_counts1), cor_stats) %>%
    summarize_all(median) %>%
    mutate(Run = .y)
})

metrics_meds_cs23 <- metrics_runs_cs23 %>%
  gather(key = "Metric", value = "Expression", -Run) %>%
  group_by(Metric) %>%
  mutate(
    Min = min(Expression),
    ...
    Max = max(Expression),
    SD = sd(Expression),
    Median_lab = paste0("Median = ", scales::number(median(Expression), accuracy = 0.001))
  ) %>%
  ungroup()

p_meds_cs23 <- ggplot(metrics_meds_cs23, aes(Expression)) +
  geom_histogram(bins = 30, fill = "blue") +
  geom_text(aes(x = 0.8, y = 90, label = Median_lab),
            hjust = 0,
            vjust = 1,
            check_overlap = TRUE) +
  facet_grid(cols = vars(Metric)) +
  labs(y = "Count",
       title = "CS2 vs. CS3 Random1 Normalized 100 Runs of Median Concordance Measures") +
  theme_bw() +
  theme(panel.grid.minor = element_blank())
print(p_meds_cs23)
```
...
```{r metrics-summ-cs23}
metrics_summ_cs23 <- metrics_meds_cs23 %>% 
  arrange(Metric) %>% 
  select(-c(Run, Expression, Median_lab)) %>% 
  distinct()

knitr::kable(metrics_summ_cs23, 
             caption = "CS2 vs CS3 Random1 Normalized 100 Runs of Summary Concordance Measures",
             digits = 3)
```
...
```{r cs23-dsc}
set.seed(2021)
cs23_batch <- rep(2:3, each = nrow(cs3_counts1))
cs23_dsc <- list(
  `CS2-None vs CS3` = cs2_counts1,
  `CS2-Random1 vs CS3` = cs2_norm_rand1,
  `CS2-Pools vs CS3` = cs2_counts1_pools
) %>%
  map(bind_rows, cs3_counts1) %>% 
  map_dfr(DSC, Y = cs23_batch, permut = 1000, .id = "Comparison")

knitr::kable(cs23_dsc, caption = "DSC for CS2 vs CS3 Comparisons")
```
...
```{r cs-chain-rand1-cs1v3-2, fig.cap='CS1 CodeSet Chaining Concordance Measure Distributions 2'}
counts1v3 <-
  list(cs1_counts1, cs1_norm_rand1_cs3, cs12_rand1_pools, cs3_common_rand1) %>%
  set_names(c("CS1-Non", "CS1-Random1", "CS1-Random1+Pools", "CS3")) %>% 
  map(select, names(cs3_common_rand1))
...
metrics_rand1v3 <- comps1v3 %>%
  ...
  mutate(Median = paste0("Median = ", scales::number(median(Expression), accuracy = 0.01))) %>%
  ungroup() %>% 
  mutate(Sites = fct_relevel(Sites, "CS1-Random1", after = 3L))
...
p_rand1v3 <- ggplot(metrics_rand1v3, aes(Expression)) +
  ...
  facet_grid(rows = vars(Sites), cols = vars(Metric), scales = "free_x") +
  labs(y = "Count",
       title = "CS1 CodeSet Chaining Concordance Measure Distributions 2") +
  theme_bw() +
  theme(panel.grid.minor = element_blank())
print(p_rand1v3)
```
...
```{r cs13-dsc}
set.seed(2021)
cs13_batch <- rep(c(1, 3), each = nrow(counts1v3[["CS3"]]))
cs13_dsc <- list(
  `CS1-None vs CS3` = counts1v3[["CS1-Non"]],
  `CS1-Random1+Pools vs CS3` = counts1v3[["CS1-Random1+Pools"]],
  `CS1-Random1 vs CS3` = counts1v3[["CS1-Random1"]]
) %>% 
  map(bind_rows, counts1v3[["CS3"]]) %>% 
  map_dfr(DSC, Y = cs13_batch, permut = 1000, .id = "Comparison")

knitr::kable(cs13_dsc, caption = "DSC for CS1 vs CS3 Comparisons")
```

```{r cs13-genes-topbot}
counts1v3_df <- counts1v3 %>%
  map_dfr(~ {
    rownames_to_column(.x, "ottaID") %>%
      gather("Gene", "Expression", -ottaID)
  },
  .id = "Data") %>%
  mutate(Gene = fct_inorder(Gene)) %>%
  spread(Data, Expression) %>% 
  gather(Norm, CS1, matches("CS1"))

counts1v3_df_topbot <- counts1v3_df %>% 
  group_by(Norm, Gene) %>% 
  summarize(Rc = epiR::epi.ccc(CS1, CS3)[["rho.c"]][["est"]]) %>% 
  filter(dense_rank(Rc) <= 3 | dense_rank(desc(Rc)) <= 3) %>% 
  mutate(Rc_lab = paste0("Rc = ", round(Rc, 3))) %>% 
  ungroup() %>% 
  inner_join(counts1v3_df, by = c("Norm", "Gene")) %>% 
  droplevels()
```

```{r cs13-genes-topbot-non, fig.cap='Pairwise Genes by Top & Bottom 3 Rc for CS1-Non vs. CS3'}
p_cs13_genes_topbot_non <-
  ggplot(counts1v3_df_topbot %>%
           filter(Norm == "CS1-Non") %>%
           mutate(Gene = fct_drop(fct_reorder(Gene, Rc))),
         aes(x = CS1, y = CS3)) + 
  geom_point() +
  geom_abline(intercept = 0, slope = 1, color = "red") +
  geom_text(aes(label = Rc_lab), x = -12, y = 2, size = 3, 
            hjust = 0, check_overlap = TRUE) +
  facet_wrap(vars(Gene), nrow = 2, ncol = 3) +
  scale_x_continuous(breaks = c(-8, -4, 0), limits = c(-12, 4)) +
  lims(y = c(-10, 3)) +
  labs(x = "CS1-Non",
       title = "Pairwise Genes by Top & Bottom 3 Rc for CS1-Non vs. CS3") +
  theme_bw() +
  theme(panel.grid.minor = element_blank())
print(p_cs13_genes_topbot_non)
```

```{r cs13-genes-topbot-rand1, fig.cap='Pairwise Genes by Top & Bottom 3 Rc for CS1-Random1 vs. CS3'}
p_cs13_genes_topbot_rand1 <-
  ggplot(counts1v3_df_topbot %>%
           filter(Norm == "CS1-Random1") %>%
           mutate(Gene = fct_drop(fct_reorder(Gene, Rc))),
         aes(x = CS1, y = CS3)) + 
  geom_point() +
  geom_abline(intercept = 0, slope = 1, color = "red") +
  ...
  lims(y = c(-10, 3)) +
  labs(x = "CS1-Random1",
       title = "Pairwise Genes by Top & Bottom 3 Rc for CS1-Random1 vs. CS3") +
  theme_bw() +
  theme(panel.grid.minor = element_blank())
print(p_cs13_genes_topbot_rand1)
```

```{r cs13-genes-topbot-rand1pools, fig.cap='Pairwise Genes by Top & Bottom 3 Rc for CS1-Random1+Pools vs. CS3'}
p_cs13_genes_topbot_rand1pools <-
  ggplot(counts1v3_df_topbot %>%
           filter(Norm == "CS1-Random1+Pools") %>%
           mutate(Gene = fct_drop(fct_reorder(Gene, Rc))),
         aes(x = CS1, y = CS3)) + 
  geom_point() +
  geom_abline(intercept = 0, slope = 1, color = "red") +
  ...
  scale_x_continuous(breaks = c(-8, -4, 0), limits = c(-12, 4)) +
  lims(y = c(-10, 3)) +
  labs(x = "CS1-Random1+Pools",
       title = "Pairwise Genes by Top & Bottom 3 Rc for CS1-Random1+Pools vs. CS3") +
  theme_bw() +
  theme(panel.grid.minor = element_blank())
print(p_cs13_genes_topbot_rand1pools)
```
...
## Normalize CS5 to CS3 using Set A pools
# Weighted mean gene expression for CS5 pools (using CS3 pools weights)
cs5_poolsA_mgx <- cs3_weights %>%
  imap_dfc(~ .x * rowMeans(select(cs5_poolsA, matches(.y)))) %>%
  transmute(Name = cs5_poolsA[["Name"]], cs5_exp = rowSums(.))

# Normalize each gene by adding batch effect (diff in mean gx)
cs5_norm_A <-
  inner_join(cs5_poolsA_mgx, cs3_pools_mgx, by = "Name") %>%
  transmute(Name, be = cs3_exp - cs5_exp) %>%
  inner_join(cs5_norm_counts, by = "Name") %>%
  gather(FileName, exp, -1:-2) %>%
  transmute(Name = fct_inorder(Name), FileName, exp = be + exp) %>%
  spread(FileName, exp) %>% 
...
counts5_BA <-
  list(cs35_norm, cs5_norm_counts, cs5_norm_B, cs5_norm_BA, cs5_norm_A) %>%
  set_names(c("CS3-Non", "CS5-Non", "CS5-NormB", "CS5-NormBA", "CS5-NormA")) %>% 
...
comps5_BA <- tibble(
  ...
  `CS3-Non_vs_CS5-NormBA` = c("CS3-Non", "CS5-NormBA"),
)
...
```{r cs-chain-cs5-setBA-2, fig.cap='CS5 Set B/A Chaining Concordance  Measure Distributions 2', fig.height=6}
comps5_BA <- tibble(
  `CS3-Non_vs_CS5-Non` = c("CS3-Non", "CS5-Non"),
  `CS3-Non_vs_CS5-NormBA` = c("CS3-Non", "CS5-NormBA"),
...
p_pools5_BA <- ggplot(metrics_pools5_BA, aes(Expression)) +
  ...
  facet_grid(rows = vars(Sites), cols = vars(Metric), scales = "free_x") +
  labs(y = "Count",
       title = "CS5 Set B/A Chaining Concordance Measure Distributions 2",
       subtitle = cs345_subtitle) +
  theme_bw() +
  theme(panel.grid.minor = element_blank())
print(p_pools5_BA)
...
```{r cs35-BA-dsc}
set.seed(2021)
cs35_BA_batch <- rep(c(3, 5), each = nrow(counts5_BA[["CS3-Non"]]))
cs35_BA_dsc <- list(
  `CS3 vs CS5-None` = counts5_BA[["CS5-Non"]],
  `CS3 vs CS5-NormBA` = counts5_BA[["CS5-NormBA"]],
  `CS3 vs CS5-NormA` = counts5_BA[["CS5-NormA"]]
) %>% 
  map(bind_rows, counts5_BA[["CS3-Non"]]) %>% 
  map_dfr(DSC, Y = cs35_BA_batch, permut = 1000, .id = "Comparisons")

knitr::kable(cs35_BA_dsc, caption = "DSC for CS3 vs CS5 Set B/A Comparisons")
```
...
counts5_CA <-
  list(cs35_norm, cs5_norm_counts, cs5_norm_C, cs5_norm_CA, cs5_norm_A) %>%
  set_names(c("CS3-Non", "CS5-Non", "CS5-NormC", "CS5-NormCA", "CS5-NormA")) %>% 
  map_at(-1, ~ gather(., File.Name.CS5, exp, -Name) %>% 
           spread(Name, exp) %>% 
           inner_join(cs45_overlap, by = "File.Name.CS5") %>% 
           select(-matches("File.Name")) %>% 
...
comps5_CA <- tibble(
  ...
  `CS3-Non_vs_CS5-NormCA` = c("CS3-Non", "CS5-NormCA"),
)
...
```{r cs-chain-cs5-setCA-2, fig.cap='CS5 Set C/A Chaining Concordance  Measure Distributions 2', fig.height=6}
comps5_CA <- tibble(
  `CS3-Non_vs_CS5-Non` = c("CS3-Non", "CS5-Non"),
  `CS3-Non_vs_CS5-NormCA` = c("CS3-Non", "CS5-NormCA"),
  `CS3-Non_vs_CS5-NormA` = c("CS3-Non", "CS5-NormA")
...
p_pools5_CA <- ggplot(metrics_pools5_CA, aes(Expression)) +
  ...
  facet_grid(rows = vars(Sites), cols = vars(Metric), scales = "free_x") +
  labs(y = "Count",
       title = "CS5 Set C/A Chaining Concordance Measure Distributions 2",
       subtitle = cs345_subtitle) +
  theme_bw() +
  theme(panel.grid.minor = element_blank())
print(p_pools5_CA)
...
```{r cs35-CA-dsc}
set.seed(2021)
cs35_CA_batch <- rep(c(3, 5), each = nrow(counts5_CA[["CS3-Non"]]))
cs35_CA_dsc <- list(
  `CS3 vs CS5-None` = counts5_CA[["CS5-Non"]],
  `CS3 vs CS5-NormCA` = counts5_CA[["CS5-NormCA"]],
  `CS3 vs CS5-NormA` = counts5_CA[["CS5-NormA"]]
) %>% 
  map(bind_rows, counts5_CA[["CS3-Non"]]) %>% 
  map_dfr(DSC, Y = cs35_CA_batch, permut = 1000, .id = "Comparisons")

knitr::kable(cs35_CA_dsc, caption = "DSC for CS3 vs CS5 Set C/A Comparisons")
```
...
## Histotype Distribution in Classifier Datasets
...
```{r train-hist}
train_hist <- count(train_ref, revHist, sort = TRUE) %>% 
  mutate(freq = percent(n / sum(n), accuracy = 1))

knitr::kable(train_hist, caption = "Full Training Set Histotype Distribution")
```
...
knitr::kable(train_hist_codeset, caption = "Full Training Set Histotype Distribution by CodeSet")
```

```{r cs1-all-hist}
cs1_all_hist <- count(cs1_all_ref, revHist, sort = TRUE) %>% 
  mutate(freq = percent(n / sum(n), accuracy = 1))

knitr::kable(cs1_all_hist, caption = "CS1 All Training Set Histotype Distribution")
```

```{r cs2-all-hist}
cs2_all_hist <- count(cs2_all_ref, revHist, sort = TRUE) %>% 
  mutate(freq = percent(n / sum(n), accuracy = 1))

knitr::kable(cs2_all_hist, caption = "CS2 All Training Set Histotype Distribution")
```

```{r conf-hist}
conf_hist <- count(conf_ref, revHist, sort = TRUE) %>% 
  mutate(freq = percent(n / sum(n), accuracy = 1))

knitr::kable(conf_hist, caption = "Confirmation Set Histotype Distribution")
```

```{r val-hist}
val_hist <- count(val_ref, revHist, sort = TRUE) %>% 
  mutate(freq = percent(n / sum(n), accuracy = 1))

knitr::kable(val_hist, caption = "Validation Set Histotype Distribution")
```
