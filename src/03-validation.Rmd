...
cs4 <- read_rcc(here("data-raw/OTTA2017_C6082/rcc"))
cs5 <- read_rcc(here("data-raw/OTTA2018_C6830/rcc"))
cs6 <- read_rcc(here("data-raw/OTTA2018_C8440/rcc"))
```
<!--
## Concordance
...
# Pool SetA and SetB from CS4
cs4_dat <- HKnorm(cs4$raw)
cs4_pools <- cs4_dat %>% 
  select(Name, matches("pool", ignore.case = TRUE))
cs4_poolsA <- cs4_pools %>% 
  select(Name, matches("Pool ?[1-3](?!0|1)|[A-C].*", perl = TRUE)) %>% 
  rename(Pool1A = `Pool A`, Pool2B = `Pool B`, Pool3C = `Pool C`)
cs4_poolsB <- cs4_pools %>% 
  select(Name, matches("Pool ?(9|10|11).*")) %>% 
  column_to_rownames("Name")

# Pool SetB from CS5
cs5_dat <- HKnorm(cs5$raw) %>% 
  mutate(Name = ifelse(Name == "CD3E", "CD3e", Name))
...
# Normalize each gene by adding batch effect (diff in mean gx)
cs5_norm_B <-
  inner_join(cs5_pools_mgx, cs4_pools_mgx, by = "Name") %>%
...
# Normalize each gene by adding batch effect (diff in mean gx)
cs5_norm_A <-
  inner_join(cs4_pools_mgx, cs3_pools_mgx, by = "Name") %>%
...
cs4_norm_counts <- cs4_dat %>% select(-one_of(names(cs4_pools)[-1]))
...
```{r cs-chain-cs5-setBA, fig.cap='CS5 Set B/A Chaining Concordance  Measure Distributions'}
cs35_norm <- cs3_norm_van %>%
  select(-one_of(names(ref_pools))) %>%
  gather(FileName, exp, -1:-3) %>%
  mutate(FileName = gsub("^X", "", FileName)) %>%
  inner_join(hist, by = "FileName") %>% 
  filter(Name %in% cs5_norm_counts$Name,
         ottaID %in% names(cs5_norm_counts)[-1]) %>% 
...
cs34_norm <- cs3_norm_van %>%
  select(-one_of(names(ref_pools))) %>%
  gather(FileName, exp, -1:-3) %>%
  mutate(FileName = gsub("^X", "", FileName)) %>%
  inner_join(hist, by = "FileName") %>%
  filter(Name %in% cs4_norm_counts$Name,
         ottaID %in% gsub("-N1|\\.(x|y)", "", names(cs4_norm_counts)[-1:-3])) %>%
...
counts4_A <-
  ...
  map_at(-1, ~ rename(., ottaID = FileName) %>% 
           mutate(ottaID = gsub("-N1|\\.(x|y)", "", ottaID)) %>% 
           semi_join(cs34_norm, by = "ottaID") %>% 
           group_by(ottaID) %>% 
...
cs_45_norm <- cs4_norm_counts %>% 
  semi_join(cs5_norm_counts, by = "Name") %>% 
  select(-matches("\\.y")) %>% 
  rename_all(~ gsub("-N1|\\.x", "", .)) %>% 
  select(any_of(names(cs5_norm_counts))) %>% 
  mutate(Name = factor(Name, levels = cs5_norm_counts$Name)) %>% 
  arrange(Name) %>% 
  gather(ottaID, exp, -Name) %>% 
  spread(Name, exp)

counts45_B <-
  list(cs_45_norm, cs5_norm_counts, cs5_norm_B) %>% 
  set_names(c("CS4-Non", "CS5-Non", "CS5-NormB")) %>% 
  map_at(-1, ~ gather(., ottaID, exp, -Name) %>%
           spread(Name, exp) %>%
           semi_join(cs_45_norm, by = "ottaID")) %>% 
  map(column_to_rownames, "ottaID")
...
cs4_poolsC <- cs4_pools %>% 
  select(Name, matches("Pool ?[4-8].*")) %>% 
  column_to_rownames("Name")

# Pool SetC from CS5
cs5_poolsC <- select(cs5_pools, Name, matches("POOL[4-8]"))

## Normalize CS5 to CS4 using Set C pools
# CS4 Set B pools weights
cs4_weightsC <- paste0("Pool ?", 4:8) %>%
  set_names() %>%
  map_dbl(~ mean(grepl(., names(cs4_poolsC), ignore.case = TRUE)))

# Weighted mean gene expression for CS5 pools (using CS4 pools weights)
cs5_pools_mgx <- cs4_weightsC %>%
...
```{r cs-chain-cs5-setCA, fig.cap='CS5 Set C/A Chaining Concordance  Measure Distributions'}
cs35_norm <- cs3_norm_van %>%
  select(-one_of(names(ref_pools))) %>%
  gather(FileName, exp, -1:-3) %>%
  mutate(FileName = gsub("^X", "", FileName)) %>%
...
counts45_C <-
  list(cs_45_norm, cs5_norm_counts, cs5_norm_C) %>% 
...
cs4_overlap_cs5 <-
  inner_join(
    cs4$exp %>% mutate(ottaID = gsub("-N1$", "", File.Name)),
    cs5$exp %>% mutate(ottaID = File.Name),
    by = "ottaID",
    suffix = c(".CS4", ".CS5")
  ) %>%
  select(ottaID, File.Name.CS4, File.Name.CS5)

# CS3 overlaps 45 CS4/5
cs3_overlap_cs45 <-
  inner_join(annot_cs_all, cs4_overlap_cs5, by = "ottaID") %>%
  filter(CodeSet == "CS3") %>%
  select(ottaID, File.Name.CS4, File.Name.CS5, CodeSet, revHist) %>%
  count(CodeSet, revHist) %>%
  pivot_wider(names_from = "CodeSet", values_from = "n") %>% 
  add_column(CS4 = 45L, CS5 = 45L)
knitr::kable(cs3_overlap_cs45, caption = "CS3 and CS4/5 Common Samples Histotype Distribution")
```
...
cs5_pool_counts <- paste0("Pool", 1:3) %>%
  set_names() %>%
  map_df(~ sum(grepl(., names(cs5_pools), ignore.case = TRUE)))
...
knitr::kable(cs345_pools, caption = "CS3CS4/CS5 Pools Distribution")
```
