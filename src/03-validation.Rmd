...
# Normalize by reference method using common samples, add histotypes from annot
cs1_norm_rand <-
  refMethod(cs1_counts, cs1_rand, cs3_rand) %>%
  as.data.frame() %>%
  rownames_to_column("ottaID") %>%
  inner_join(hist %>% distinct(ottaID, revHist), by = "ottaID") %>%
  column_to_rownames("ottaID")
...
cs2_norm_rand <-
  refMethod(cs2_counts, cs2_rand, cs3_rand) %>%
  as.data.frame() %>%
  rownames_to_column("ottaID") %>%
  inner_join(hist %>% distinct(ottaID, revHist), by = "ottaID") %>%
  column_to_rownames("ottaID")
...
cs3_norm_rand <-
  refMethod(cs3_counts, cs3_rand, cs2_rand) %>%
  as.data.frame() %>%
  rownames_to_column("ottaID") %>%
  inner_join(hist %>% distinct(ottaID, revHist), by = "ottaID") %>%
  column_to_rownames("ottaID")
...
```{r hgsc3-table}
tmp <- list(cs1_norm_rand, cs2_norm_rand, cs3_norm_rand) %>%
  set_names(codesets) %>% 
  map(pivot_longer, names_to = "gene", values_to = "exp", cols = -revHist)
```
