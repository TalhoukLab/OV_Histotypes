...
counts1v3_df_topbot_non <- counts1v3_df %>% 
  group_by(Norm, Gene) %>% 
  summarize(Rc = epiR::epi.ccc(CS1, CS3)[["rho.c"]][["est"]]) %>% 
  filter(Norm == "CS1-Non",
         dense_rank(Rc) <= 3 | dense_rank(desc(Rc)) <= 3) %>% 
  ungroup()

counts1v3_df_topbot_norms <- counts1v3_df %>% 
  group_by(Norm, Gene) %>% 
  summarize(Rc = epiR::epi.ccc(CS1, CS3)[["rho.c"]][["est"]]) %>% 
  filter(Norm != "CS1-Non") %>% 
  semi_join(counts1v3_df_topbot_non, by = "Gene")

counts1v3_df_topbot <- bind_rows(counts1v3_df_topbot_non,
                                 counts1v3_df_topbot_norms) %>%
  mutate(Rc_lab = paste0("Rc = ", round(Rc, 3))) %>%
  inner_join(counts1v3_df, by = c("Norm", "Gene")) %>%
  droplevels()
```

```{r cs13-genes-pairwise, fig.cap='Pairwise Genes by Top/Bottom 3 Rc for CS1 vs. CS3'}
p_cs13_genes_topbot <-
  ggplot(counts1v3_df_topbot, aes(x = CS1, y = CS3)) + 
  geom_point() +
  geom_abline(intercept = 0, slope = 1, color = "red") +
  geom_text(aes(label = Rc_lab), x = -12, y = 2, size = 3, 
            hjust = 0, check_overlap = TRUE) +
  facet_grid(rows = vars(Norm), cols = vars(Gene)) +
  labs(title = "Pairwise Genes by Top/Bottom 3 Rc for CS1 vs. CS3") +
  theme_bw() +
  theme(panel.grid.minor = element_blank())
print(p_cs13_genes_topbot)
```
...
cs345_overlap <- annot_cs %>% 
  filter(CodeSet == "CS3") %>% 
  select(ottaID, File.Name.CS3 = FileName) %>% 
  inner_join(cs45_overlap, by = "ottaID")

cs345_common_samples <- unique(cs345_overlap$ottaID)
...
counts5_BA <-
  ...
  map_at(-1, ~ gather(., File.Name.CS5, exp, -Name) %>% 
           spread(Name, exp) %>% 
           inner_join(cs345_overlap, by = "File.Name.CS5") %>% 
...
counts4_A <-
  ...
  map_at(-1, ~ inner_join(., cs345_overlap,
