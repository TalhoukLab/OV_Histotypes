...
cs3_norm_rand3 <-
  refMethod(cs3_counts3, cs2_rand3, cs3_rand3) %>%
  as.data.frame() %>%
  rownames_to_column("ottaID") %>%
  inner_join(hist %>% distinct(ottaID, revHist), by = "ottaID") %>%
  column_to_rownames("ottaID")
...
cs3_norm_rand2 <-
  refMethod(cs3_counts2, cs2_rand2, cs3_rand2) %>%
  as.data.frame() %>%
  rownames_to_column("ottaID") %>%
  inner_join(hist %>% distinct(ottaID, revHist), by = "ottaID") %>%
  column_to_rownames("ottaID")
...
cs3_norm_rand1 <-
  refMethod(cs3_counts1, cs2_rand1, cs3_rand1) %>%
  as.data.frame() %>%
  rownames_to_column("ottaID") %>%
  inner_join(hist %>% distinct(ottaID, revHist), by = "ottaID") %>%
  column_to_rownames("ottaID")
...
cs3_norm_rand <-
  refMethod(cs3_counts, cs2_rand, cs3_rand) %>%
  as.data.frame() %>%
  rownames_to_column("ottaID") %>%
  inner_join(hist %>% distinct(ottaID, revHist), by = "ottaID") %>%
  column_to_rownames("ottaID")

norm_rand <- list(cs1_norm_rand, cs2_norm_rand, cs3_norm_rand) %>%
  set_names(codesets) %>%
  map(select, -"revHist")

metrics_rand <- all_codesets %>%
  imap_dfr(~ {
    pmap_dfr(norm_rand[.x], ~ {
      R2 <- cor(.x, .y) ^ 2
      ccc <- epiR::epi.ccc(.x, .y)
      Ca <- pluck(ccc, "C.b")
      Rc <- pluck(ccc, "rho.c", "est")
...
```{r cc-by-hist}
cs1_byhist <- cs1_norm_rand %>%
  split(.$revHist) %>%
  map(select, -"revHist")
cs2_byhist <- cs2_norm_rand %>%
  split(.$revHist) %>%
  map(select, -"revHist")
cs3_byhist <- cs3_norm_rand %>%
  split(.$revHist) %>%
  map(select, -"revHist")
```

```{r cs1-vs-cs3-by-hist}
cs1_vs_cs3 <- list(cs1_byhist, cs3_byhist) %>%
  transpose() %>%
  map_dfr(function(x)
    pmap_df(x, ~ {
      R2 <- cor(.x, .y) ^ 2
...
knitr::kable(cs1_vs_cs3, caption = "CS1 vs. CS3 Median Concordance Measures by Histotypes")
```

```{r cs2-vs-cs3-by-hist}
cs2_vs_cs3 <- list(cs2_byhist, cs3_byhist) %>%
  transpose() %>%
  map_dfr(function(x)
    pmap_df(x, ~ {
      R2 <- cor(.x, .y) ^ 2
...
knitr::kable(cs2_vs_cs3, caption = "CS2 vs. CS3 Median Concordance Measures by Histotypes")
```
...
all_comps <- tibble(
  ...
  `CS2Non_vs_CS2Pools` = c("CS2Non", "CS2Pools"),
  `CS3Non_vs_CS3Pools` = c("CS3Non", "CS3Pools")
)
common_gx <-
  set_names(
    list(
...
metrics_pools <- all_comps %>%
  ...
  mutate(Median = paste0("Median = ", scales::number(median(Expression), accuracy = 0.01))) %>%
  ungroup() %>% 
  mutate(Sites = factor(Sites, levels = c(
    "CS2Non_vs_CS2Pools",
    "CS3Non_vs_CS3Pools",
    "CS2Non_vs_CS3Non",
    "CS2Pools_vs_CS3Pools"
  )))
```

```{r cc-pools, fig.cap='Pools Normalized Concordance Measure Distributions', fig.height=7}
