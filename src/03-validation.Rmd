...
```{r rand1-cs1-vs-cs3}
rand1_cs1_vs_cs3 <- list(cs1_norm_rand1_byhist, cs3_counts1_byhist) %>%
  transpose() %>%
  map_dfr(function(x)
    pmap_df(x, ~ {
      R2 <- cor(.x, .y) ^ 2
...
```{r rand1-cs2-vs-cs3}
rand1_cs2_vs_cs3 <- list(cs2_norm_rand1_byhist, cs3_counts1_byhist) %>%
  transpose() %>%
  map_dfr(function(x)
    pmap_df(x, ~ {
      R2 <- cor(.x, .y) ^ 2
...
rand3_hgsc_cs1_vs_cs3 <- list(cs1_byhist, cs3_byhist) %>%
  transpose() %>%
  map_dfr(function(x)
    pmap_df(x, ~ {
      R2 <- cor(.x, .y) ^ 2
...
rand3_hgsc_cs2_vs_cs3 <- list(cs2_byhist, cs3_byhist) %>%
  transpose() %>%
  map_dfr(function(x)
    pmap_df(x, ~ {
      R2 <- cor(.x, .y) ^ 2
...
# CS2 normalized: 894 samples (903 original - 9 from pools), 136 common genes # dim(cs2_normalized_data_pools)
...
```{r cc-pools, fig.cap='Pools Normalized Concordance Measure Distributions', fig.height=6}
...
pools_cs2non_vs_cs3 <- list(cs2_common_non_byhist, cs3_common_non_byhist) %>%
  transpose() %>%
  map_dfr(function(x)
    pmap_df(x, ~ {
      R2 <- cor(.x, .y) ^ 2
...
pools_cs2norm_vs_cs3 <- list(cs2_common_pools_byhist, cs3_common_non_byhist) %>%
  transpose() %>%
  map_dfr(function(x)
    pmap_df(x, ~ {
      R2 <- cor(.x, .y) ^ 2
