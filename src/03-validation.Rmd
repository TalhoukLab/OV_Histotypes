...
# Normalize by reference method using common samples, add histotypes from annot
cs1_norm_rand1 <-
  refMethod(cs1_counts1, cs2_rand1, cs1_rand1) %>%
  as.data.frame() %>%
  rownames_to_column("ottaID") %>%
...
cs1_cs2_norm_counts <- cs2_norm %>%
  ...
  inner_join(cs1_norm_rand1, by = "Name")
...
```{r cs-chain-rand1}
counts1 <- cs1_norm_rand1 %>%
  pivot_longer(names_to = "ottaID", values_to = "exp", -1) %>%
  pivot_wider(names_from = "Name", values_from = "exp") %>%
  column_to_rownames("ottaID") %>%
  list(cs1_counts1, cs2_counts1, .) %>%
