...
# Up-XGB optimal gene list workflow
set.seed(2024)
...
# Up-XGB optimal gene list model trained
set.seed(2024)
...
```{r}
#| label: tbl-val-eval
#| tbl-cap: 'Evaluation Metrics on Validation Set Model, Up-XGB, Full Set'
#| tbl-pos: 'H'
...
val_pred_overall <- data.frame(FileName = rownames(val_data),
                               Truth = val_class) |>
  mutate(
    Prediction_up_xgb_full = predict(mod_up_xgb, val_data)[[".pred_class"]],
    Prediction_up_xgb_opt = predict(mod_up_xgb_opt, val_data)[[".pred_class"]],
    across(c("Truth", matches("Prediction")), factor)
  ) |> 
  pivot_longer(
    cols = matches("Prediction"),
...
```{r}
#| label: fig-val-roc-up-xgb-full
#| fig-cap: 'ROC Curves for Up-XGB, Full Set Model in Validation Set'
#| fig-height: 7
#| fig-width: 8
#| out-width: '100%'
...
auc <- preds |>
  ...
  pull(.estimate) |> 
  number(accuracy = 0.001, prefix = "AUC = ")
val_roc_curve_up_xgb_full <- 
  autoplot(roc_df) +
  labs(title = "ROC Curve for Up-XGB, Full Set Model in Validation Set",
       subtitle = auc) +
  facet_wrap2(~ .level, strip = strip_themed(background_x = elem_list_rect(fill = class_palette))) +
  theme(
    panel.grid = element_blank(),
...
print(val_roc_curve_up_xgb_full)
```

### Up-XGB, Optimal Set

```{r}
#| label: fig-val-roc-up-xgb-opt
#| fig-cap: 'ROC Curves for Up-XGB, Optimal Set Model in Validation Set'
#| fig-height: 7
#| fig-width: 8
#| out-width: '100%'

preds <- mod_up_xgb_opt |> 
  predict(val_data, type = "prob") |> 
  add_column(truth = factor(val_class))
roc_df <- preds |>
  roc_curve(truth, matches(".pred")) |> 
...
auc <- preds |>
  ...
  pull(.estimate) |> 
  number(accuracy = 0.001, prefix = "AUC = ")
val_roc_curve_up_xgb_opt <- 
  autoplot(roc_df) +
  labs(title = "ROC Curve for Up-XGB, Optimal Set Model in Validation Set",
       subtitle = auc) +
  facet_wrap2(~ .level, strip = strip_themed(background_x = elem_list_rect(fill = class_palette))) +
  theme(
    panel.grid = element_blank(),
...
print(val_roc_curve_up_xgb_opt)
```
:::
...
```{r}
#| label: fig-val-calib-up-xgb-full
#| fig-cap: 'Calibration Plots for Up-XGB, Full Set Model in Validation Set'
#| fig-height: 7
#| fig-width: 8
#| out-width: '100%'
...
preds |> 
  ...
  labs(title = "Calibration Plots for Up-XGB, Full Set Model in Validation Set")
```

### Up-XGB, Optimal Set

```{r}
#| label: fig-val-calib-up-xgb-opt
#| fig-cap: 'Calibration Plots for Up-XGB, Optimal Set Model in Validation Set'
#| fig-height: 7
#| fig-width: 8
#| out-width: '100%'
...
preds |> 
  ...
  theme(panel.grid.minor = element_blank(),
        strip.text = element_text(face = "bold"),
        strip.placement = "inside") +
  labs(title = "Calibration Plots for Up-XGB, Optimal Set Model in Validation Set")
```
:::
