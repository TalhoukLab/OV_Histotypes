...
p_gene_up_xgb <- 
  ...
  labs(
    x = "Genes Added",
    y = "Balanced Accuracy",
    title = "Gene Optimziation for Up-XGB Classifier"
  )
...
In the Up-XGB classifier, the mean balanced accuracy is highest when we reach `r n_genes_up_xgb` genes added, hence the optimal number of genes used will be n=`r length(overlap_all)`+`r n_genes_up_xgb`=`r length(opt_genes_up_xgb)` The added genes are: `r str_flatten_comma(genes_add_up_xgb, last = " and ")`.
