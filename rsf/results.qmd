...
#| fig-cap: 'Gene Optimization for SMOTE-SVM Classifier using Balanced Accuracy'
#| fig-height: 5
#| fig-width: 5
#| out-width: '100%'
...
dat_gene_smote_svm <- gene_opt_de_all_metrics_conf |> 
...
In the SMOTE-SVM classifier, the mean balanced accuracy is optimal with `r n_genes_smote_svm` genes added, hence the optimal number of genes used will be n=`r length(overlap_all)`+`r n_genes_smote_svm`=`r length(opt_genes_smote_svm)` The added genes are: `r str_flatten_comma(genes_add_smote_svm, last = " and ")`.
