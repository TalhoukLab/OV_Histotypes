...
#| fig-cap: 'Gene Optimization for SMOTE-RF Classifier'
#| fig-height: 5
#| fig-width: 5
#| out-width: '100%'
...
dat_gene_smote_rf <- gene_opt_all_metrics_conf |> 
  ...
  mutate(label = paste0("Workflow: SMOTE-RF, Measure: ", .metric)) |> 
...
p_gene_smote_rf <- 
  ...
  labs(x = "Genes Added",
       y = "F1-Score",
       title = "Gene Optimziation for SMOTE-RF Classifier")
...
In the SMOTE-RF classifier, the mean F1-score is highest when we reach `r n_genes_smote_rf` genes added, hence the optimal number of genes used will be n=`r length(overlap_all)`+`r n_genes_smote_rf`=`r length(opt_genes_smote_rf)` The added genes are: `r str_flatten_comma(genes_add_smote_rf, last = " and ")`.

```{r}
#| label: tbl-gene-opt-smote-rf
#| tbl-cap: 'Gene Profile of Optimal Set in SMOTE-RF Workflow'
#| tbl-pos: 'H'
...
gene_dist_smote_rf <-
  tibble(
    Genes = gene_order_smote_rf,
    Set = ifelse(Genes %in% overlap_all, "Base", "Candidates"),
    PrOTYPE = ifelse(Genes %in% overlap_PrOTYPE, symbol$tick, ""),
    SPOT = ifelse(Genes %in% overlap_SPOT, symbol$tick, ""),
    `Optimal Set` = if_else(
      Genes %in% opt_genes_smote_rf,
      symbol$circle_filled,
      symbol$circle_cross
    ) |>
      cell_spec(
...
conf_pred_overall <- data.frame(FileName = rownames(conf_data),
  ...
  mutate(Method = fct_inorder(
    case_match(
      ...
      "smote_rf_full" ~ "SMOTE-RF, Full Set",
      "smote_rf_opt" ~ "SMOTE-RF, Optimal Set",
...
conf_entropy <- data.frame(FileName = rownames(conf_data),
  ...
  mutate(Method = fct_inorder(
    case_match(
      ...
      "p_smote_rf_full" ~ "SMOTE-RF, Full Set",
      "p_smote_rf_opt" ~ "SMOTE-RF, Optimal Set",
...
#| fig-cap: 'ROC Curves for SMOTE-RF, Full Set Model in Confirmation Set'
#| fig-height: 7
#| fig-width: 8
#| out-width: '100%'
...
roc_curve_smote_rf_full <-
  autoplot(roc_df) +
  labs(title = "ROC Curve for SMOTE-RF, Full Set Model in Confirmation Set",
       subtitle = auc) +
...
#| fig-cap: 'ROC Curves for SMOTE-RF, Optimal Set Model in Confirmation Set'
#| fig-height: 7
#| fig-width: 8
#| out-width: '100%'
...
roc_curve_smote_rf_opt <- 
  autoplot(roc_df) +
  labs(title = "ROC Curve for SMOTE-RF, Optimal Set Model in Confirmation Set",
       subtitle = auc) +
  facet_wrap2(~ .level, strip = strip_themed(background_x = elem_list_rect(fill = class_palette))) +
  theme(
    panel.grid = element_blank(),
...
```{r}
#| label: tbl-val-eval
#| tbl-cap: 'Evaluation Metrics on Validation Set Model, SMOTE-RF, Optimal Set'
#| tbl-pos: 'H'
...
#| fig-cap: 'SMOTE-RF Per-Class Metrics in Validation Set'
#| fig-width: 8
#| out-width: '100%'
...
p <- ggplot(val_metrics_top) +
  ...
  labs(
    y = "Metric Value",
    title = paste("SMOTE-RF Per-Class Metrics in Validation Set")
  )
print(p)
```
...
#| fig-cap: 'ROC Curves for SMOTE-RF, Optimal Set Model in Validation Set'
#| fig-height: 7
#| fig-width: 8
#| out-width: '100%'
...
val_roc_curve_smote_rf_opt <- 
  autoplot(roc_df) +
  labs(title = "ROC Curve for SMOTE-RF, Optimal Set Model in Validation Set",
       subtitle = auc) +
  facet_wrap2(~ .level, strip = strip_themed(background_x = elem_list_rect(fill = class_palette))) +
  theme(
    panel.grid = element_blank(),
