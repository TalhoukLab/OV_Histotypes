...
### Top Training Set Workflows

```{r}
#| label: tbl-conf-metrics-top
#| tbl-cap: "Top Workflow Per-Class Evaluation Metrics in Confirmation Set"

all_metrics_conf <- all_metrics_conf |> 
  separate(wflow, c("Subsampling", "Algorithms"), remove = FALSE) |> 
  mutate(
    Subsampling = factor(Subsampling, levels = c("none", "down", "up", "smote", "hybrid")),
    Algorithms = factor(Algorithms, levels = c("rf", "svm", "xgb", "mr")),
...
per_class_metrics_conf_seq <- per_class_metrics_conf_seq |>
  mutate(class_group = factor(class_group, levels = c("HGSC", "CCOC", "ENOC", "LGSC", "MUC"))) |>
  mutate(
    lower = ifelse(all(is.na(.estimate)), NA, min(.estimate, na.rm = TRUE)),
    upper = ifelse(all(is.na(.estimate)), NA, max(.estimate, na.rm = TRUE)),
...
per_class_metrics_conf_two_step <- per_class_metrics_conf_two_step |>
  mutate(class_group = factor(class_group, levels = c("HGSC", "CCOC", "ENOC", "LGSC", "MUC"))) |>
  mutate(
    lower = ifelse(all(is.na(.estimate)), NA, min(.estimate, na.rm = TRUE)),
    upper = ifelse(all(is.na(.estimate)), NA, max(.estimate, na.rm = TRUE)),
...
# Combine all per-class metrics
per_class_metrics_conf <- bind_rows(all_metrics_conf,
                                    per_class_metrics_conf_seq,
                                    per_class_metrics_conf_two_step) |>
  filter(class_group != "Overall") |>
  droplevels()

metrics_conf_top <- per_class_metrics_conf |> 
  distinct(pick(-fold_id, -.estimate)) |> 
  rename(Workflow = wflow) |> 
  inner_join(wflow_rank_agg_metrics, by = "Workflow") |> 
  mutate(
...
metrics_conf_top_df <- metrics_conf_top |>
  select(Metric = .metric, Workflow, mean_estimate, lower, upper, class_group) |>
  mutate(across(where(is.numeric), ~ round(., digits = 3))) |>
  mutate(Estimate = paste0(mean_estimate, " (", lower, ", ", upper, ")"),
         .keep = "unused") |> 
...
mod_seq_opt <- list(all_models_seq,
  ...
  pmap(~ {
    ...
    set.seed(2024)
    wflow |>
      fit(data = cbind(..2, class = factor(..3)))
  })
...
# up_xgb full model trained
set.seed(2024)
mod_up_xgb_full <- wflow_up_xgb_model |>
  fit(data = train_ref)

# up_xgb optimal gene list workflow
set.seed(2024)
...
# up_xgb optimal gene list model trained
set.seed(2024)
...
conf_pred_overall <- data.frame(FileName = rownames(conf_data),
  ...
  mutate(
    c(p_seq_full = mod_seq_full,
      ...
      p_up_xgb_full = list(mod_up_xgb_full),
      p_up_xgb_opt = list(mod_up_xgb_opt)
    ) |> 
  ...
  mutate(
    ...
    Prediction_up_xgb_full = p_up_xgb_full,
    Prediction_up_xgb_opt = p_up_xgb_opt,
  ...
  mutate(Method = fct_inorder(
    case_match(
      ...
      "two_step_full" ~ "2-STEP, Full Set",
      "two_step_optimal" ~ "2-STEP, Optimal Set",
      "up_xgb_full" ~ "Up-XGB, Full Set",
      "up_xgb_opt" ~ "Up-XGB, Optimal Set"
    )
  ))
...
conf_entropy <- data.frame(FileName = rownames(conf_data),
  ...
  mutate(
    c(p_seq_full = mod_seq_full,
      ...
      p_two_step_full = mod_two_step_full,
      p_two_step_opt = mod_two_step_opt,
      p_up_xgb_full = list(up_xgb_full = mod_up_xgb_full),
      p_up_xgb_opt = list(up_xgb_opt = mod_up_xgb_opt)
    ) |>
      map(~ {
  ...
  mutate(Method = fct_inorder(
    case_match(
      ...
      "p_two_step_full" ~ "2-STEP, Full Set",
      "p_two_step_opt" ~ "2-STEP, Optimal Set",
      "p_up_xgb_full" ~ "Up-XGB, Full Set",
      "p_up_xgb_opt" ~ "Up-XGB, Optimal Set"
    )
  )) |>
...
#| fig-cap: 'ROC Curves for 2-STEP Full Model in Confirmation Set'
#| fig-height: 7
#| fig-width: 9
#| out-width: '100%'
...
roc_curve_two_step_full <- mod_two_step_full |> 
  ...
  wrap_plots(widths = c(1, -1)) +
  plot_annotation(
    title = "ROC Curves for 2-STEP, Full Set Model in Confirmation Set",
    tag_levels = "A"
  )
...
#| fig-cap: 'ROC Curves for 2-STEP Optimal Model in Confirmation Set'
#| fig-height: 7
#| fig-width: 9
#| out-width: '100%'
...
roc_curve_two_step_optimal <- mod_two_step_opt |> 
  ...
  wrap_plots(widths = c(1, -1)) +
  plot_annotation(
    title = "ROC Curves for 2-STEP, Optimal Set Model in Confirmation Set",
    tag_levels = "A"
  )
...
```{r}
#| label: fig-roc-curve-up-xgb-full
#| fig-cap: 'ROC Curves for Up-XGB, Full Set Model in Confirmation Set'
#| fig-height: 7
#| fig-width: 8
#| out-width: '100%'

preds <- mod_up_xgb_full |> 
  predict(conf_data, type = "prob") |>
...
auc <- preds |>
  ...
  pull(.estimate) |> 
  number(accuracy = 0.001, prefix = "AUC = ")
roc_curve_up_xgb_full <-
  autoplot(roc_df) +
  labs(title = "ROC Curve for Up-XGB, Full Set Model in Confirmation Set",
       subtitle = auc) +
...
print(roc_curve_up_xgb_full)
```

#### Up-XGB, Optimal

```{r}
#| label: fig-roc-curve-up-xgb-opt
#| fig-cap: 'ROC Curves for Up-XGB, Optimal Set Model in Confirmation Set'
#| fig-height: 7
#| fig-width: 8
#| out-width: '100%'
...
auc <- preds |>
  ...
  pull(.estimate) |> 
  number(accuracy = 0.001, prefix = "AUC = ")
roc_curve_up_xgb_opt <- 
  autoplot(roc_df) +
  labs(title = "ROC Curve for Up-XGB, Optimal Set Model in Confirmation Set",
       subtitle = auc) +
  facet_wrap2(~ .level, strip = strip_themed(background_x = elem_list_rect(fill = class_palette))) +
  theme(
    panel.grid = element_blank(),
...
val_pred_overall <- data.frame(FileName = rownames(val_data),
                               Truth = val_class) |>
  mutate(
    Prediction_up_xgb_opt = predict(mod_up_xgb_full, val_data)[[".pred_class"]],
    across(c("Truth", matches("Prediction")), factor)
  ) |> 
  pivot_longer(
    cols = matches("Prediction"),
...
val_pred_hgsc_samples <- mod_up_xgb_full |> 
