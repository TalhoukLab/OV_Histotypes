...
conf_lgsc_condition <- conf_pred_overall |>
  filter(Method == "SMOTE-SVM", Truth == "LGSOC") |>
...
conf_lgsc_exp <- cs3 |>
  rename_all(~ gsub("^X", "", .)) |>
  select(Name, all_of(conf_lgsc_condition[["FileName"]])) |>
  column_to_rownames("Name")

dds_conf_lgsc <- DESeqDataSetFromMatrix(countData = conf_lgsc_exp,
                                        colData = conf_lgsc_condition,
                                        design = ~ Prediction_LSGC)
dds_conf_lgsc <- dds_conf_lgsc[rowSums(counts(dds_conf_lgsc)) > 1, ]
dds_conf_lgsc <- DESeq(dds_conf_lgsc)

res_conf_lgsc <- dds_conf_lgsc |>
  results() |>
  as.data.frame() |>
  rownames_to_column(var = "Gene") |>
  as_tibble() |>
  filter(Gene %in% names(train_data), !Gene %in% overlap_all) |>
  arrange(padj, desc(abs(log2FoldChange))) |>
  mutate(Rank = seq_along(Gene), .keep = "used") |>
  add_column(wflow = "smote_svm", .before = 1)

# ENOC differential gene expression analysis in confirmation set
conf_enoc_condition <- conf_pred_overall |>
  filter(Method == "SMOTE-SVM", Truth == "ENOC") |>
...
conf_enoc_exp <- cs3 |>
  rename_all(~ gsub("^X", "", .)) |>
  select(Name, all_of(conf_enoc_condition[["FileName"]])) |>
  column_to_rownames("Name")

dds_conf_enoc <- DESeqDataSetFromMatrix(countData = conf_enoc_exp,
                                        colData = conf_enoc_condition,
                                        design = ~ Prediction_ENOC)
dds_conf_enoc <- dds_conf_enoc[rowSums(counts(dds_conf_enoc)) > 1, ]
dds_conf_enoc <- DESeq(dds_conf_enoc)

res_conf_enoc <- dds_conf_enoc |>
  results() |>
  as.data.frame() |>
  rownames_to_column(var = "Gene") |>
  as_tibble() |>
...
conf_de_rank <-
  bind_rows(LGSC = res_conf_lgsc, ENOC = res_conf_enoc, .id = "class") |>
  pivot_wider(names_from = "class", values_from = "Rank") |>
  mutate(Variable = Gene,
         Rank = row_number(rowMeans(pick(LGSC, ENOC))),
         .keep = "unused") |>
  arrange(Rank)
...
In the SMOTE-SVM classifier, the optimal number of genes is achieved at the fewest genes needed to achieve a mean F1-Score above 0.7 with `r n_genes_smote_svm` genes added, hence the optimal number of total genes used will be n=`r length(overlap_all)`+`r n_genes_smote_svm`=`r length(opt_genes_smote_svm)` The added genes are: `r str_flatten_comma(genes_add_smote_svm, last = " and ")`.
