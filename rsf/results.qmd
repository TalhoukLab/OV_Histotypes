...
all_supp_tables <- bind_rows(
  ...
  mutate(List = fct_inorder(List))

# All genes
marina_genes_total <- all_supp_tables |> 
  filter(List %in% c("Total genes, All studies",
                     "Total genes, Reduced studies")) |> 
  pull(Genes) |> 
  unique()
...
res_lgsc <- dds_lgsc |>
  ...
  mutate(
    Significant = if_else(padj < 0.05, TRUE, FALSE),
    `Marina's Sets` = if_else(Gene %in% marina_genes_total, TRUE, FALSE)
  )

# LGSC volcano plot
val_lgsc_volcano <-
  ggplot(res_lgsc, aes(x = log2FoldChange, y = -log10(padj))) +
  geom_point(aes(shape = Significant, color = `Marina's Sets`),
             alpha = 0.5,
             stroke = 0.75) +
  geom_text_repel(data = res_lgsc |> filter(Significant),
                  aes(label = Gene),
                  seed = 2025) +
  ...
  theme(panel.grid.minor = element_blank()) +
  labs(title = "LGSC",
       caption = "Significant: adjusted p-value < 0.05",
       x = "Log2 Fold Change",
       y = "-Log10 p-value")
...
res_enoc <- dds_enoc |>
  ...
  rownames_to_column(var = "Gene") |>
  mutate(
    Significant = if_else(padj < 1e-4, TRUE, FALSE),
    `Marina's Sets` = if_else(Gene %in% marina_genes_total, TRUE, FALSE)
  )

# ENOC volcano plot
val_enoc_volcano <-
  ggplot(res_enoc, aes(x = log2FoldChange, y = -log10(padj))) +
  geom_point(aes(shape = Significant, color = `Marina's Sets`),
             alpha = 0.5,
             stroke = 0.75) +
  geom_text_repel(
    data = res_enoc |> filter(Significant),
    aes(label = Gene),
    seed = 2025,
    max.overlaps = 25
  ) +
  ...
  theme(panel.grid.minor = element_blank()) +
  labs(title = "ENOC",
       caption = "Significant: adjusted p-value < 0.0001",
       x = "Log2 Fold Change",
       y = "-Log10 p-value")

# Combined volcano plots
val_all_volcano <-
  list(val_lgsc_volcano, val_enoc_volcano) |>
  wrap_plots(ncol = 2) +
  plot_layout(guides = "collect",
              axes = "collect") +
  plot_annotation(title = "Volcano Plots of Validation Set Predictions",
                  tag_levels = "A")
