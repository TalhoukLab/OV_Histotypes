...
val_conf_mat_simp + val_roc_curve_up_xgb_full + val_eval_gg +
...
val_lgsc_condition <- val_pred_overall |>
  filter(Method == "Up-XGB, Full", Truth == "LGSOC") |>
  mutate(Prediction_LSGC = fct_other(
    Prediction,
    keep = "LGSOC",
    other_level = "non_LGSOC"
...
val_enoc_condition <- val_pred_overall |>
  filter(Method == "Up-XGB, Full", Truth == "ENOC") |>
  mutate(Prediction_ENOC = fct_other(
    Prediction,
    keep = "ENOC",
    other_level = "non_ENOC"
...
val_all_volcano <- ggplot(res_all, aes(x = log2FoldChange, y = -log10(padj))) +
  ...
  labs(
    ...
    title = "Volcano Plots of Validation Set Predictions using Up-XGB, Full Set",
...
bind_rows(
  ...
  select(Dataset, FileName, GAD1, LGALS4, hist_final) |>
...
val_pred_hgsc_samples <- mod_up_xgb |> 
