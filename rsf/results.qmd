...
In the sequential algorithm, all sequences have relatively flat average F1-scores across the number of genes added. One area we can use to differentiate is in sequence 3, where the maximum F1-score is highest when we reach `r n_genes_seq` genes added, hence the optimal number of genes used will be n=`r length(overlap_all)`+`r n_genes_seq`=`r length(opt_genes_seq)` The added genes are: `r str_flatten_comma(genes_add_seq, last = " and ")`.
...
#| fig-cap: 'Gene Optimization for 2-STEP Classifier'
#| fig-height: 6
#| fig-width: 5
#| out-width: '100%'
...
p_gene_two_step <-
  ...
  labs(x = "Genes Added",
       y = "F1-Score",
       title = "Gene Optimziation for 2-STEP Classifier")
...
#| tbl-cap: 'Gene Profile of Optimal Set in 2-STEP Workflow'
#| tbl-pos: 'H'
...
gene_dist_two_step <-
  tibble(
    Genes = gene_order_two_step,
    Set = ifelse(Genes %in% overlap_all, "Base", "Candidates"),
    PrOTYPE = ifelse(Genes %in% overlap_PrOTYPE, symbol$tick, ""),
    SPOT = ifelse(Genes %in% overlap_SPOT, symbol$tick, ""),
    `Optimal Set` = if_else(Genes %in% opt_genes_two_step,
                            symbol$circle_filled,
                            symbol$circle_cross) |> 
      cell_spec(
        color = case_when(
...
dat_gene_up_xgb <- gene_opt_all_metrics_conf |> 
  ...
  mutate(label = paste0("Workflow: Up-XGB, Measure: ", .metric)) |> 
...
dat_gene_smote_svm <- gene_opt_all_metrics_conf |> 
  ...
  mutate(label = paste0("Workflow: SMOTE-SVM, Measure: ", .metric)) |> 
...
p_gene_smote_svm <- 
  ...
  labs(x = "Genes Added",
       y = "F1-Score",
       title = "Gene Optimziation for SMOTE-SVM Classifier")
...
In the SMOTE-SVM classifier, the mean F1-score is gets close to 0.7 when we reach `r n_genes_up_xgb` genes added, hence the optimal number of genes used will be n=`r length(overlap_all)`+`r n_genes_up_xgb`=`r length(opt_genes_up_xgb)` The added genes are: `r str_flatten_comma(genes_add_up_xgb, last = " and ")`.
