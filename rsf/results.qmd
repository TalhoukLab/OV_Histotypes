...
p <- ggplot(
  all_metrics_train %>% filter(.metric == "gmean"),
  aes(
    x = fct_reorder(wflow, mean_estimate, .desc = TRUE),
    y = mean_estimate,
    ymin = lower,
...
```{r}
#| label: fig-metrics-top-by-metric-all
...
```{r}
#| label: fig-metrics-top-by-metric-3
#| fig-cap: !expr paste('Top', n_wflow_top, 'Workflow Per-Class Evaluation Metrics by Metric')
#| fig-width: 10
#| fig-height: 6
#| out-width: '100%'
...
metrics_top <- per_class_metrics |> 
  ...
  rename(Workflow = wflow) |> 
  inner_join(wflow_rank_agg_metrics, by = "Workflow") |> 
  filter(.metric %in% c("f_meas", "bal_accuracy", "kap")) |> 
  mutate(
    Workflow = fct_reorder(Workflow, Rank),
    .metric = case_match(
      .metric,
...
candidates_all <- gene_seq_names %>%
  filter(Genes > 0L) %>% 
  pull(Name)
candidates_unused <- gene_seq_names %>%
  filter(Genes > 0L) %>% 
  slice_max(order_by = Genes, n = -n_genes_seq) |> 
  arrange(Genes) |> 
  pull(Name)
gene_order <- c(opt_genes_seq, candidates_unused)

gene_dist <-
  data.frame(Genes = gene_order) |>
...
candidates_all <- gene_smote_rf_names %>%
  filter(Genes > 0L) %>% 
  pull(Name)
candidates_unused <- gene_smote_rf_names %>%
  filter(Genes > 0L) %>% 
  slice_max(order_by = Genes, n = -n_genes_smote_rf) |> 
  arrange(Genes) |> 
  pull(Name)
gene_order <- c(opt_genes_smote_rf, candidates_unused)
...
candidates_all <- gene_two_step_names %>%
  filter(Genes > 0L) %>% 
  pull(Name)
candidates_unused <- gene_two_step_names %>%
  filter(Genes > 0L) %>% 
  slice_max(order_by = Genes, n = -n_genes_two_step) |> 
  arrange(Genes) |> 
  pull(Name)
gene_order <- c(opt_genes_two_step, candidates_unused)
...
mod_seq_full <- list(all_models_seq,
                     seq_data,
                     seq_class) |> 
  pmap(~ ..1 |> fit(data = cbind(..2, class = ..3)))
...
mod_seq_opt <- list(all_models_seq,
  ...
  pmap(~ {
    ..1 |>
      ...
      step_select(c(all_of(!!opt_genes_seq), class), skip = TRUE) |>
      modify_in("var_info", \(x) x |>
...
# smote_rf full model
set.seed(2024)
mod_smote_rf_full <- smote_rf_model |>
  fit(data = train_ref)
...
mod_smote_rf_opt <- smote_rf_model |> 
  ...
  update_recipe(x = smote_rf_model, recipe = _) |> 
  fit(data = train_ref)
...
mod_two_step_opt <- list(all_models_two_step,
  ...
  pmap(~ {
    ..1 |>
      pluck("pre", "actions", "recipe", "recipe") |>
      step_select(c(all_of(!!opt_genes_two_step), class), skip = TRUE) |>
      modify_in("var_info", \(x) x |>
...
conf_entropy <- data.frame(FileName = rownames(conf_data),
  ...
  filter(
    (grepl("Sequential", Method) & (
      Prediction == "HGSC" & Workflow == "smote_rf_s1" |
        Prediction == "CCOC" & Workflow == "hybrid_xgb_s2" |
        Prediction == "ENOC" & Workflow == "up_svm_s3" |
        Prediction %in% c("LGSC", "MUC") & Workflow == "hybrid_mr_s4"
    )) |
    grepl("SMOTE-Random Forest", Method) |
    (grepl("Two-Step", Method) & (
      Prediction == "HGSC" & Workflow == "smote_rf_s1" |
        Prediction != "HGSC" & Workflow == "hybrid_svm_s2"
    ))
  ) |>
