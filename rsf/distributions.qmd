...
train_cohort_codeset <- hist_all_mol |>
  ...
  droplevels() |>
  mutate(CodeSet = factor(CodeSet, levels = c("CS1", "CS2", "CS3"))) |> 
  describeBy(
    var.names = "hist_gr2",
...
train_hist_codeset <- hist |>
  mutate(
    hist_final = factor(hist_final, levels = c("HGSOC", "CCOC", "ENOC", "MUOC", "LGSOC")),
    CodeSet = factor(CodeSet, levels = c("CS1", "CS2", "CS3"))
  ) |> 
  describeBy(
...
```{r}
#| label: tbl-tissue-src
#| tbl-cap: 'Tissue Source by Cohort'

all_ref <-
  bind_rows(Training = train_ref,
            Confirmation = conf_ref,
            Validation = val_ref,
            .id = "Data") |>
  rownames_to_column("FileName") |>
  mutate(col_name = paste0("X", FileName),
         Data = factor(Data, levels = c("Training", "Confirmation", "Validation")))

tissue_src <- cs_all_df |> 
  inner_join(all_ref, by = "col_name") |> 
  mutate(Cohort = fct_recode(cohort, OVAR11 = "ICON7")) |> 
  count(Data, Cohort, tissue.source) |>
  pivot_wider(
    names_from = "tissue.source",
    values_from = "n",
...
combined_cohorts <- bind_rows(Training = train_coh,
                              Confirmation = tnco,
                              Validation = dove4, .id = "Cohort") |>
  mutate(FileName = gsub("^X", "", col_name), .before = col_name) |>
  mutate(Cohort = fct_inorder(Cohort))

qc_failed <- 
  bind_rows(cs1_qc, cs2_qc, cs3_qc) |>
  droplevels() |>
  inner_join(combined_cohorts, by = "FileName") |>
  tbl_summary(
    by = Cohort,
    label = list(
      linFlag ~ "Linearity",
...
genes_cs13_rand1 <- train_ref_comb |>
  ...
  group_by(Gene) |>
  mutate(Rc = epi.ccc(Value_CS1, Value_CS3)[["rho.c"]][["est"]]) |>
  ungroup() |>
...
genes_cs23_rand1 <- train_ref_comb |>
  ...
  group_by(Gene) |>
  mutate(Rc = epi.ccc(Value_CS2, Value_CS3)[["rho.c"]][["est"]]) |>
  ungroup() |>
  mutate(Gene = fct_reorder(Gene, Rc, .desc = TRUE),
         Rc_label = paste0("Rc = ", round(Rc, digits = 3)))
...
genes_comp_test <-
  ...
  bold_labels() |>
  modify_header(label ~ "**Correlation**") |>
  add_p(
